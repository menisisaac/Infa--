...
void Lexer::NumberState(std::string line, bool hasSign) {
  ...
  while (index < length && (isdigit(line.at(index)) || line.at(index) == '.')) {
    if (line.at(index) == '.') {
      ...
      "Multiple Decimals at line " + to_string(lineNumber) + " position " +
          to_string(index);
      hasDecimal = true;
    } else {
      seenNumber = true;
  ...
  if (seenNumber) {
    if (hasDecimal) {
      if (!seenNumberAfterDecimal) {
        std::cout << "Error"; // runtime_error("Must have number after decimal,
                              // use f to indicate float instead");
      }
    }
...
void Lexer::StringState(std::string line) {
  ...
  while (index < length) {
    if (line.at(index) == '\"' || line.at(index) == '"') {
      tokens.push_back(
          new Token(String, line.substr(start + 1, index - start - 1)));
      ++index;
      break;
    }
    ++index;
